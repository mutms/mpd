...
# --- CA mirror logic ---
# Two real-file locations, mirrored on every run:
#   ~/Developer/mpd/conf/caroot/{rootCA.pem,rootCA-key.pem} — canonical,
#       shared with mpd-desktop on macOS hosts. Only populated when
#       ~/Developer/mpd/conf/ already exists.
#   ~/.mpd-machine/ca/{rootCA.pem,rootCA-key.pem} — platform-owned.
#       Always populated after first setup.sh.
# Wiping either side restores from the other on the next run.
