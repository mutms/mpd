...
|---|---|---|---|
| **macOS + UTM** (Apple Silicon) | **Ships** | [`macos-utm/`](macos-utm/README.md) | Double-click `setup.command`: downloads the Debian cloud image, prepares cloud-init, imports into UTM, builds `bin/mpd` inside the VM, and configures macOS networking (route, NRPT-equivalent DNS resolver, CA trust). `start.command` / `stop.command` / `uninstall.command` cover the lifecycle. |
...
| **Ubuntu + KVM** (Linux desktop) | **Parked** | [`ubuntu-kvm/`](ubuntu-kvm/README.md) | Future third polished platform: `setup.sh` driving QEMU with KVM acceleration via libvirt's default network, configuring the host with `ip route` + systemd-resolved drop-in + `update-ca-certificates`, plus pre-warm symmetric with the other automated platforms. Use [`generic-vm/`](generic-vm/README.md) in the meantime. |
