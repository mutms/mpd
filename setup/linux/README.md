...
the user's first `mpd start` is faster (the runtime container itself is
already built by `mpd --vm-setup`). Best-effort — a failure here just
means lazy provisioning at first project start.
