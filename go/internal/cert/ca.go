...
// CA before a VM exists: `CA.swift` in mpd-virt-macos, and the
// `generate_mpd_ca` function in setup/linux/lib/common.sh. All three
// must produce a certificate with an identical DN, v3_ca extensions and
// name constraints, because a VM reuses whichever one it finds.
//
