...
func podService() Service {
	return Service{
		...
		PodContainers: []PodContainer{
			{Suffix: "postgres", Image: "postgres:16", Volume: "mpd-svc-authentik-db", VolumePath: "/data",
				RunArgs: []string{"-e", "POSTGRES_DB=authentik"}},
			{Suffix: "server", Primary: true, Image: "authentik:latest", Args: []string{"server"}},
		},
	}
}
...
func TestPodHelpers(t *testing.T) {
	...
	if got := s.PodName(); got != "mpd-svc-authentik" {
		t.Errorf("PodName = %q", got)
	}
	if got := s.memberName(s.PodContainers[0]); got != "mpd-svc-authentik-postgres" {
		t.Errorf("memberName = %q", got)
	}
	vols := s.Volumes()
	if len(vols) != 1 || vols[0] != "mpd-svc-authentik-db" {
		t.Errorf("Volumes = %v, want [mpd-svc-authentik-db]", vols)
	}
...
func TestCreatePodArgs(t *testing.T) {
	...
	for _, want := range []string{
		"pod create", "--name mpd-svc-authentik",
	...
	for _, want := range []string{
		"run -d --pod mpd-svc-authentik", "--name mpd-svc-authentik-postgres",
		"--restart always", "-v mpd-svc-authentik-db:/data",
		"mpd.name=authentik-postgres", // helper is suffixed, not the service name
		"POSTGRES_DB=authentik", "postgres:16",
	} {
	...
	for _, want := range []string{
		"--name mpd-svc-authentik-server", "authentik:latest server",
	} {
	...
	if !strings.Contains(server, "--label mpd.name=authentik ") {
		t.Errorf("server run %q should carry mpd.name=authentik", server)
	}
