...
// Reconcile rewrites the record files that mpd derives rather than
// writes on demand: the service records handed in by the caller, and
// the database names read back from live containers. Stale out-of-zone
...
// record file is already live by the time this returns — there is
// nothing to signal, restart or wait for.
//
// verbose is off on the daily path, where "nothing moved" is the normal
// answer and saying so on every `--vm-start` is noise.
func Reconcile(ctx context.Context, out io.Writer, m Manager,
	records []ServiceRecord, vmIP string, verbose bool) error {
