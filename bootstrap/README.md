...
| `30-mpd-build.sh` | no | Creates `/opt/mpd` + `/var/lib/mpd` owned by the dev user; clones or fast-forwards the mpd repo; installs upstream Go into `/usr/local/go` when the VM has none; `make install`; puts `/opt/mpd/bin` and `~/.local/bin` on PATH via `~/.bashrc`. |
