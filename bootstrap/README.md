...
| `30-networking.sh` | no (local) | no | Hostname rename to `mpd-<NNN>`, systemd-resolved + libnss-resolve, optional static IP for octets in `[100, 254]` via `/etc/systemd/network/05-mpd.network`, IPv6 disable, write `/var/lib/mpd/conf/platform.env`. Requires systemd-networkd (template prep must remove NetworkManager). |
