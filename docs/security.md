...
`mpd-caddy.service` on `.2` serves the per-project certs — and a `TLS`
service's frontdoor cert for `<name>.caddy.<zone>`, issued and written to
`/srv/meta/<name>/` the same way. Their keys are `0600` and dev-owned,
which is why that unit runs as the dev user. `caddy.service` on `.1`
serves the service cert for the portal. A service reached only at its raw
`<name>.svc` address (no `TLS`) has no certificate: it is plain HTTP
