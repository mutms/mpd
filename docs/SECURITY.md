...
| Property                | Value                                                           |
|-------------------------|-----------------------------------------------------------------|
| Host-side location      | `~/.mpd/conf/caroot/rootCA.pem` + `rootCA-key.pem` (on macOS)   |
...
`~/.mpd/conf/wireguard/` (mode `0700` directory, `0600` private key).
...
|-------------------------|------------------------------------------------|-------------------|
| CA private key          | `~/.mpd/conf/caroot/rootCA-key.pem` (host)     | `0600`            |
| WireGuard keys          | `~/.mpd/conf/wireguard/` (host)                | `0700` directory  |
...
The host's `~/.mpd/conf/` is preserved by design (so a re-setup
...
- **Physical access to the host**: anyone with access to
  `~/.mpd/conf/` can read the CA key and the WireGuard private key.
