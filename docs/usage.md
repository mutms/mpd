...
an mdl-demo container and vice versa. Every project's backups share that one
directory, so a bundle can be restored into any project — name yours to tell
them apart, and `mdl-data-restore --list` shows the recipe and database each
came from. It skips the source tree (the recipe rebuilds it; that is git's
job), the PHPUnit and Behat dataroots (`phpunit-init` / `behat-init` rebuild
them), and Moodle's caches, sessions and trash.

Backups work for any engine the project runs (PostgreSQL, MariaDB, MySQL).
An mdl-demo container ships PostgreSQL 17 only, so `mdl-data-backup` prints a
note when the bundle is not PostgreSQL 17 or older — it will not restore
there.
...
PostgreSQL dump cannot load into MariaDB — and one taken on a newer engine
version than the project runs (a plain dump only loads forward, into an
equal-or-newer server); it tells you which `MPD_DB` value to set and retry.
These same checks make a cross-project restore safe: it is just a restore
whose bundle happens to come from another project.
