...
| `mpci` / `mpci-install`                     | Moodle Plugin CI runner and installer. Lives at `/opt/mpd/mpci/` (dev-owned, in the container overlay — re-provision after `mpd --runtime-rebuild`, like `~/.nvm`).                                                                                                                                               |
